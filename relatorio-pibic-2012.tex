...
\hyphenation{o-pe-ra-\c{c}\~{o}es}
\hyphenation{a-pri-mo-ra-men-to}
...
% Marcos: o resumo está ótimo. Mude apenas a pessoa. use a primeira
% pessoa do singular: "Utilizei os materiais, cumpri as etapas,
% revisei as operações, etc."
\resumo{Contorno musical consiste numa linha que marca movimentos como
  melodia, ritmo, timbre, etc. São importantes por serem mais
  perceptíveis que notas e por ajudarem a dar coerência a uma obra
  musical.
  % Marcos: fale que existe uma teoria de contornos, que há uma
  % inconsistência, que eu estou revisando a teoria e que você
  % desenvolveu o mapa de operações de contornos usadas nas análises
  % de composições nas referências bibliográficas.
  Devido uma inconsistência que encontrou-se nesse trabalho
  foi desenvolvido um mapa de operações de contornos musicais para
  economizar tempo e energia na busca por tais operações.

  Durante a construção do mapa de operações utilizou-se os seguintes
  materiais: Mapa de operações, Mendeley, MusiCountour, Python,
  Linux/Ubuntu, Git, Github e \LaTeX/Kile. Para realização do trabalho
  foram cumpridas as seguintes etapas: Familiarização com o objeto da
  pesquisa, alimentação da planilha de operações, treinamento com
  ferramentas Linux/Ubuntu, Python e MusiContour e revisão das
  operações mapeadas.

  O principal resultado deste trabalho é o mapa das operações de
  contornos musicais presentes nas referências bibliográficas sobre o
  tema contornos musicais. Os demais resultados foram o aprofundamento
  do conhecimento sobre contornos, aprendizado sobre Python,
  aprendizado sobre MusiContour, aprendizado sobre \LaTeX/Kile,
  aprendizado sobre Linux/Ubuntu, aprimoramento da escrita de textos
  acadêmicos, aprimoramento da leitura em língua inglesa,
  aprimoramento de organização e técnicas de estudo.

  Após o término do mapeamento de operações revisou-se todas as
  operações contidas. Foram encontradas inconsistências em algumas
  operações. Estas inconsistências terão análises posteriores, pois a
